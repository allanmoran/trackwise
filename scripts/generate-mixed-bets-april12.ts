#!/usr/bin/env node
/**
 * April 12 - Scrape form cards and generate mixed bets
 */
...
const urlsToScrape = [
  ...
  'https://www.sportsbetform.com.au/436172/3309982/',
  // Terang (8 races)
...
async function main() {
  console.log('\n🏇 APRIL 12 - SCRAPING FORM CARDS & GENERATING MIXED BETS\n');
  ...
  const topWinners = settlementResults.filter(r => r.pnl > 0).sort((a, b) => b.pnl - a.pnl).slice(0, 10);
  for (const result of topWinners) {
    console.log(`${result.track} R${result.race}: ${result.horse} (${result.betType}) → ${result.result} +$${result.pnl}`);
  }

  console.log('\n📋 TOP LOSSES\n');
  const topLosses = settlementResults.filter(r => r.pnl < 0).sort((a, b) => a.pnl - b.pnl).slice(0, 10);
  for (const result of topLosses) {
    console.log(`${result.track} R${result.race}: ${result.horse} (${result.betType}) → ${result.result} -$${Math.abs(result.pnl)}`);
  }
}
