...
async function check() {
  ...
  console.log(`\n   Pakenham: ${pakenham[0].count}\n`);

  if (pakenham[0].count > 0) {
    const sample = await sql`
      SELECT race_num, horse, stake FROM bets WHERE track = 'Pakenham' LIMIT 5
    `;
    console.log('   Sample Pakenham bets:');
